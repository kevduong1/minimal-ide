...
export interface AgentTerminal {
  ...
  id: string;
  /** Tab label AND pane badge (one shared name; renameable from either).
   *  Defaults to the repo basename, deduped. */
  title: string;
...
/** Default badge/tab title for a terminal of this project. */
...
export const useAgentTerminalsStore = create<AgentTerminalsState>((set) => ({
  ...
  closeTerminal: (id) =>
    set((s) => {
      ...
      if (next === s) return s;
      return { ...applied(s, next), paneActivity: pruneActivity(s.paneActivity, [id]) };
    }),
